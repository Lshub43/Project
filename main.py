...
class Main(tk.Frame):
    ...
    def view_records(self):
        ...
        [self.tree.delete(i) for i in self.tree.get_children()]
        # Добавляем в виджет таблицы всю информацию из ДБ
        [self.tree.insert('', 'end', values=row) for row in self.db.c.fetchall()]
    ...
    def init_main(self):
        ...
        # command - функция понажатию
        # bg-фон
        # bd- граница
        # compound-ориентация текста (tk.CENTER, tk.LEFT, tK.RIGHT, tk.ТОР или tk.ВОТТОМ.)
        # image - иконка кнопки 
        btn_open_dialog = tk.Button(toolbar, bg='#d7d8e0', bd=0, image=self.add_img, command=self.open_dialog)
        # упаковка и выравнивание полевому краю
        btn_open_dialog.pack(side=tk.LEFT)

        # Добавлем Treeview
        # columns - столбцы
        # height - высота таблицы
        # show='headlings' скрываем нулевую (пустую) колонку таблицы
        self.tree = ttk.Treeview(self, columns=('ID', 'name', 'tel', 'email'), height=45, show='headings')

        # Добавляем параметры колонкам
        # widh - ширина
        # anchor - выпавнивание текста в ячейке
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        # меняем координаты обьекта
        self.entry_name.place(x=200, y=50)

        # Добавляем сторку ввода для email
        self.entry_email = ttk.Entry(self)
        self.entry_email.place(x=200, y=80)

        # Добавляем сторку ввода для телефона
